...
export const getToursByUser = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(404).json({ message: "User doent exits" });
  }
...
export const deleteTour = async (req, res) => {
  ...
  try {
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res.status(404).json({ message: `No tour exist width id:${id}` });
    }
    await TourModal.findByIdAndRemove(id);
    res.json({ message: "You deleted Tour successfully" });
  } catch (error) {
    res.status(404).json({ message: "Something went wrong!" });
  }
};
...
export const getToursByTag = async (req, res) => {
  ...
  try {
    const tours = await TourModal.find({ tags: { $all: [tag] } });
